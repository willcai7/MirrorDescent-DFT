...
import numpy as np
with open('./history_beta.pkl', 'rb') as f:
    data = pickle.load(f)
...
for dim in range(len(elapsed_time_history)):
    ...
    mean_times = times[:,0]
    std_times = times[:,1]
    plt.plot(betas, mean_times, label=f'{dim+1}D: ({grid_size[dim]})', color=colors[dim], linewidth=2)
...
plt.savefig('./contour_scaling_beta.png')  
