...
beta = 10
N_samples = 100
N_poless = np.linspace(0, 200, 21).astype(int)[1:]
...
for dim in range(len(Nss)):
    print(f'{dim+1}D scaling of N_poles')
    times = np.array(elapsed_time_history[dim])
    headers = ["N_poles", "Mean Time (s)", "Std Time (s)"]
    table = [[N_poless[i], times[i][0], times[i][1]] for i in range(len(N_poless))]
    print(tabulate(table, headers=headers, floatfmt=".6f"))

data = {'elapsed_time_history': elapsed_time_history, 'N_poless': N_poless}
with open('./history_N_poles.pkl', 'wb') as f:
    pickle.dump(data, f)
