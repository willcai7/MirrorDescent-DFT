...
import numpy as np
with open('./elapsed_time_history_N_samples.pkl', 'rb') as f:
    elapsed_time_history = pickle.load(f)
...
for dim in range(len(elapsed_time_history)):
    ...
    mean_times = times[:,0]
    std_times = times[:,1]
    plt.plot(N_sampless, mean_times, label=f'{dim+1}D: ({grid_size[dim]})', color=colors[dim], linewidth=2)
    plt.fill_between(N_sampless, 
...
plt.savefig('./linear_system_scaling_N_samples.png')
